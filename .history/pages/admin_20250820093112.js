...
function getTicketID(ticket) {
  if (!ticket) return '';
  return formatTicketID(ticket.ticketID ?? ticket._id ?? ticket.id ?? '');
}
...
export default function Admin() {
  ...
  const fetchTickets = async () => {
    ...
    // Map _id or id to ticketID for frontend consistency and add a stable display id
    const mapped = res.data.map(ticket => {
      // prefer immutable DB id (e.g. Mongo _id). Keep a deleteId for API calls.
      const deleteId = ticket._id ?? ticket.id ?? ticket.ticketID;
      // derive stable display id from immutable deleteId when possible, otherwise from createdAt+email fallback
      const stableSource = deleteId ?? ticket.createdAt ?? `${ticket.email ?? ''}-${ticket.subject ?? ''}`;
      return {
        ...ticket,
        // keep original ticketID for compatibility, but use deleteId when calling API
