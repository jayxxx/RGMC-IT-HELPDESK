...
const AdminTicketsPage = () => {
  ...
  const handleUpdateStatus = async (ticketId) => {
    ...
    setTickets(tickets =>
      tickets.map(t =>
        (t.id || t.ticketId) === ticketId ? { ...t, status: newStatus } : t
      )
    );
  };
        ...
        <tbody>
          {tickets.map(ticket => (
            <tr key={ticket.id || ticket.ticketId}>
              ...
              <td>
                <select
                  value={statusUpdates[ticket.id || ticket.ticketId] || ticket.status}
                  onChange={e => handleStatusChange(ticket.id || ticket.ticketId, e.target.value)}
                  style={{ marginRight: 4 }}
                >
                ...
                <button
                  onClick={() => handleUpdateStatus(ticket.id || ticket.ticketId)}
