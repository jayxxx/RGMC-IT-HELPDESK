...
// Example table rendering for paginatedTickets
<tbody>
  {paginatedTickets.map(ticket => (
    <tr key={ticket._id}>
      {/* Ticket ID: use ticketNumber from backend */}
      <td>{`TICKET-${String(ticket.ticketNumber).padStart(6, '0')}`}</td>
      {/* Other columns */}
      <td>{ticket.email}</td>
      ...
      <td>{ticket.message || <span style={{color: '#888'}}>No message</span>}</td> {/* fallback */}
      <td>{ticket.status}</td>
      <td>{ticket.updatedAt ? new Date(ticket.updatedAt).toLocaleString() : 'N/A'}</td>
      <td>
